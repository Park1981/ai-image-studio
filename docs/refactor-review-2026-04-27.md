...
| ID | 항목 | 상태 | 근거 |
|----|------|------|------|
| A | Ollama 호출 분산 → `_ollama_client.py` | **✅ 처리** | `backend/studio/_ollama_client.py` 신설. `prompt_pipeline` / `vision_pipeline` / `comparison_pipeline` / `ollama_unload` / `system_metrics` 의 Ollama HTTP 전송을 공용 helper 로 위임. |
...
```text
8. Claude A _ollama_client.py 신설 (4-6h) ✅ 완료
   → `post_json` / `get_json` / `call_chat_payload` / `get_ps` / `request_unload` 통합
   → GPU lock 완료 후 별도 patch 로 진행해 영향 범위 분리
...
핵심 미해결 (오빠 결정 필요):
  ...
  - Claude A _ollama_client.py 와 C2-P0-3 GPU lock 통합 한 번에 묶기? → 분리 진행 완료
```
...
3. Phase 1 보강 진행 순서 (라운드 3 fact-check 반영):
   ...
   - → Claude A _ollama_client.py (Phase 1 내 별도 단계) ✅ 완료
...
- `_ollama_client.py` 는 의도대로 동시 도입하지 않았고, 이후 Phase 1 별도 patch 로 완료.
