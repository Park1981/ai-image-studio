...
`_build_wan22()` 노드 그래프 (실측 워크플로우 1:1 mirror):

```
LoadImage(source_filename)
    ↓
─── HIGH NOISE 분기 ───
UnetLoaderGGUF(unet_name=files.unet_high, dequant_dtype="default")
    ↓
[Lightning ON] LoraLoaderModelOnly(name=lightning.name_high, strength=1.0)
    ↓
LoraLoaderModelOnly(name=motion.name_high, strength=0.8)
    ↓
...
KSamplerAdvanced(
    ...
    model=model_high, positive=cond_pos, negative=cond_neg,
    latent_image=wan_i2v_latent,
)
    ↓ latent_high

─── LOW NOISE 분기 ───
UnetLoaderGGUF(unet_name=files.unet_low) → Lightning LOW(토글) → motion(0.8) → ModelSamplingSD3(8.0)
    ↓ model_low
KSamplerAdvanced(
    ...
    model=model_low, positive=cond_pos, negative=cond_neg,
    latent_image=latent_high,
)
    ↓ latent_final

─── 공통 ───
CLIPLoader(clip_name=files.text_encoder, type="wan", device="default")
CLIPTextEncode(positive: prompt) / CLIPTextEncode(negative: presets.negative_prompt)
WanImageToVideo(
    width=request.width, height=request.height, length=81, batch_size=1,
    positive, negative, vae, start_image=loaded_image,
)  → wan_i2v_latent (KSamplerAdvanced.latent_image 입력)
...
VAELoader(files.vae) → VAEDecode(latent_final, vae) → frames
CreateVideo(frames, fps=16) → SaveVideo(prefix="AIS-Video", codec="h264")
```

> **⚠ 입력 파라미터명 잠정 표기**: 위 다이어그램의 `WanImageToVideo(... start_image=...)` / `KSamplerAdvanced(...)` / 슬롯 인덱스 (`vae` 출력 slot 등) 는 **잠정** 이며 Phase 1.5 캡처에서 확정. `start_image` 가 실제로는 `image` 또는 다른 이름일 수 있음 — `/object_info` 응답으로 6 node type 의 INPUT_TYPES 정확 매핑 후 빌더 코드 작성 (§6 게이트).
...
|------------|-------------|-------------|
| `UnetLoaderGGUF` | **city96/ComfyUI-GGUF** custom node | `unet_name`, `dequant_dtype` |
...
## 14. 검증 결과 박제 (Phase 6 완료 후 갱신)

> 빈 자리. 다음 항목으로 채움:
> - Phase 1.5 `_object_info_wan22.json` 발췌 + 6 node type 검증 PASS/FAIL
