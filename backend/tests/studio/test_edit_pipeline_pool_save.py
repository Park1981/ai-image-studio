...
async def test_template_pick_skips_save_to_pool(
    ...
    """라이브러리 픽 (referenceTemplateId 있음) → save_to_pool 호출 안 함, 영구 라이브러리 URL 사용."""
    import asyncio
    ...
    template_id = await history_db.insert_reference_template(
        {
            "name": "test",
            "imageRef": "/images/studio/reference-templates/perm-uuid.png",
            "roleDefault": "outfit",
    ...
    # reference_ref_url 에 영구 라이브러리 URL
    kwargs = patched_pipeline["kwargs"]
    assert kwargs.get("reference_ref_url") == "/images/studio/reference-templates/perm-uuid.png"
