...
# Phase 4.2 단계 2 — _common 그룹 분리. facade re-export.
...
# ─── JSON coerce 헬퍼 ─────────────────────────────────────────────────
# parse_strict_json / coerce_str 는 ._json_utils 에서 import (모듈 통합 · spec 19).
...
async def _call_vision_edit_source(
    ...
    *,
    ...
    if width > 0 and height > 0:
        user_lines.append(
            f"Source image attached. Aspect: {width}×{height} ({_aspect_label(width, height)})."
        )
    user_lines.append(
