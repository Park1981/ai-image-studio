"""
comparison_pipeline.v2_generic — 일반 5축 비교 (analyze_pair_generic).

Vision Compare 메뉴 전용 — 사용자가 임의로 고른 두 이미지의 일반 비교.
- 5축: composition / color / subject / mood / quality (Edit context 와 다름)
- 점수 의미: 단순 유사도 (도메인 정보 없음)
- compare_hint: 사용자 지시 힌트 (선택)

Phase 4.4 단계 4 (2026-04-30) 분리.
codex C2 fix: _COMPARE_HINT_DIRECTIVE 도 본 모듈로 (실제 사용처 _call_vision_pair_generic).
"""
